...
mod tests {
    ...
    use domain::{TaskCyclingStrategy, TaskRepository};
    ...
    async fn should_cycle_to_next_task() {
        ...
        let result =
            cycle_to_next_task(&cycling_service, Some(tasks[0].id.to_string()))
                .await
                .unwrap();
    ...
    async fn should_handle_no_tasks() {
        ...
        let result = cycle_to_next_task(&cycling_service, None).await.unwrap();
