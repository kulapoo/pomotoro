...
mod tests {
    ...
    use domain::{TaskBuilder, TaskStatus};

    async fn setup_with_tasks()
    -> (Arc<dyn TaskRepository + Send + Sync>, Vec<Task>) {
    ...
    async fn should_get_tasks_by_tags_using_domain_method() {
        ...
        let tasks = get_task_by_tags(&task_repo, vec!["personal".to_string()])
            .await
            .unwrap();
    ...
    async fn should_get_tasks_by_status_using_domain_method() {
        ...
        let tasks = get_tasks_by_status(&task_repo, TaskStatus::Completed)
            .await
            .unwrap();
