...
mod tests {
    ...
    async fn should_reset_config_to_defaults() {
        ...
        // Reset to defaults
        let reset_config =
            reset_config(&config_repo, &event_publisher).await.unwrap();
    ...
    async fn should_reset_to_factory_defaults() {
        ...
        let factory_config =
            reset_config_to_factory_defaults(&config_repo, &event_publisher)
                .await
                .unwrap();
        ...
        assert_eq!(
            factory_config.general.auto_start_breaks,
            expected_config.general.auto_start_breaks
        );
        assert_eq!(factory_config.audio.volume, expected_config.audio.volume);
    ...
    async fn should_backup_and_reset_config() {
        ...
        // Backup and reset
        let (backup, new_config) =
            backup_and_reset_config(&config_repo, &event_publisher)
                .await
                .unwrap();
