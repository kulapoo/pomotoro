...
mod tests {
    ...
    use domain::{Phase, TaskId, TimerConfiguration, TimerState, TimerStatus};
    ...
    async fn should_pause_running_timer() {
        ...
        let result = pause_timer_session(&timer_service, &event_publisher)
            .await
            .unwrap();
    ...
    async fn should_resume_paused_timer() {
        ...
        let result = resume_timer_session(&timer_service, &event_publisher)
            .await
            .unwrap();
    ...
    mod event_tests {
        ...
        async fn should_publish_timer_paused_event_when_pausing() {
            let timer_service: Arc<dyn TimerService + Send + Sync> = Arc::new(
                MockTimerService::new_with_status(TimerStatus::Running),
            );
        ...
        async fn should_not_publish_event_when_resuming() {
            let timer_service: Arc<dyn TimerService + Send + Sync> = Arc::new(
                MockTimerService::new_with_status(TimerStatus::Paused),
            );
            let mock_publisher = Arc::new(MockEventPublisher::new());
            let event_publisher =
                create_event_publisher(mock_publisher.clone());
        ...
        async fn should_not_publish_event_when_timer_stopped() {
            let timer_service: Arc<dyn TimerService + Send + Sync> = Arc::new(
                MockTimerService::new_with_status(TimerStatus::Stopped),
            );
            let mock_publisher = Arc::new(MockEventPublisher::new());
            let event_publisher =
                create_event_publisher(mock_publisher.clone());
        ...
        async fn should_publish_events_for_multiple_pause_operations() {
            ...
            pause_timer_session(&timer_service, &event_publisher)
                .await
                .unwrap();
            assert_eq!(mock_publisher.event_count(), 1);
            ...
            // Verify sequence
            assert!(
                mock_publisher.verify_event_sequence(&["Paused", "Paused"])
            );
        }
    }
}
