            ...
            loop {
                ...
                let should_transition = {
                    ...
                    if state.remaining_seconds() > 0 {
                        state.timer.remaining_seconds = state.timer.remaining_seconds.saturating_sub(1);
                ...
                if should_transition {
                    ...
                    if let Ok(result) = transition_result {
                        ...
                        let _ = app_handle.emit(timer::PHASE_COMPLETE, (&result.old_phase, &result.new_phase));
                        let _ = app_handle.emit(timer::UPDATE_STATE, state.clone());
