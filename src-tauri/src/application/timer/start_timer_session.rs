use pomotoro_domain::{TaskRepository, TaskId, Result, Error};
...
mod tests {
    use super::*;
    use pomotoro_domain::{Task, TimerState, TimerStatus};
    ...
    impl TimerService for MockTimerService {
        ...
        async fn skip_to_next_phase(&self, _task: Option<&Task>) -> Result<(pomotoro_domain::Phase, pomotoro_domain::Phase)> {
            let mut state = self.state.write().unwrap();
            state.next_phase()
        }
