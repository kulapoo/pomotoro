...
impl SettingsViewModel {
        ...
        spawn_local(async move {
            ...
            if let Ok(js_val) = serde_wasm_bindgen::to_value(&args) {
                web_sys::console::log_2(
                    &"Serialized args for SAVE_GLOBAL:".into(),
                    &js_val,
                );
            }

            invoke::<(), _>(event_names::config::SAVE_GLOBAL, Some(args))
                .await
                .map(|_| {
    ...
    pub fn update_general(&self, general: GeneralConfig) {
        ...
        spawn_local(async move {
            ...
            invoke::<Config, _>(
                event_names::config::UPDATE_GENERAL,
                Some(Args {
                    preferences: general,
                }),
            )
            ...
            .map_err(|e| {
                handle_command_error(
                    format!("Failed to update general config: {}", e),
                    set_error_state,
                );
            })
            .ok();
    ...
    pub fn update_notifications(&self, notifications: NotificationConfig) {
        ...
        spawn_local(async move {
            ...
            .map_err(|e| {
                handle_command_error(
                    format!("Failed to update notifications: {}", e),
                    set_error_state,
                );
            })
            .ok();
    ...
    pub fn update_appearance(&self, appearance: AppearanceConfig) {
        ...
        spawn_local(async move {
            ...
            invoke::<Config, _>(
                event_names::config::UPDATE_APPEARANCE,
                Some(Args {
            ...
            .map_err(|e| {
                handle_command_error(
                    format!("Failed to update appearance: {}", e),
                    set_error_state,
                );
            })
            .ok();
    ...
    pub fn update_timer(&self, timer: TimerConfiguration) {
        ...
        spawn_local(async move {
            ...
            invoke::<Config, _>(
                event_names::config::UPDATE_TIMINGS,
                Some(Args { timer }),
            )
            .await
            .map(|config| {
            ...
            .map_err(|e| {
                handle_command_error(
                    format!("Failed to update timer config: {}", e),
                    set_error_state,
                );
            })
            .ok();
    ...
    pub fn reset_to_defaults(&self) {
        ...
        spawn_local(async move {
            invoke::<Config, ()>(event_names::config::RESET_TO_DEFAULTS, None)
                .await
                .map(|config| {
                    set_config.set(Some(config));
                    // Clear any existing errors on success
    ...
    pub fn test_audio_preview(&self, sound_type: &str) {
        ...
        spawn_local(async move {
            let _ = invoke::<(), ()>(
                event_names::commands::audio::TEST_PREVIEW,
                None,
            )
            .await;
        });
