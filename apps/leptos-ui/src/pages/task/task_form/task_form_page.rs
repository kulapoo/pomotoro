...
pub fn TaskFormPage() -> impl IntoView {
    ...
    let task_id = move || {
        params.with_untracked(|p| {
            p.get("id")
                .and_then(|id| Uuid::parse_str(&id).ok().map(TaskId::from_uuid))
        })
    };
    ...
    Effect::new(move || {
        if let Some(loaded_task) = task.get() {
            ...
            if use_seconds_for_duration.get() || has_seconds_precision {
                ...
                set_short_break
                    .set(timer_config.short_break_duration.as_secs());
                set_long_break.set(timer_config.long_break_duration.as_secs());
            } else {
                set_work_duration
                    .set(timer_config.work_duration.as_secs() / 60);
                set_short_break
                    .set(timer_config.short_break_duration.as_secs() / 60);
                set_long_break
                    .set(timer_config.long_break_duration.as_secs() / 60);
            }
        ...
        if use_custom_config.get() {
                ...
                || work_secs > MAX_WORK_DURATION.as_secs()
            {
                return Err(format!(
                    ...
                    if is_seconds_mode {
                        MIN_WORK_DURATION.as_secs()
                    } else {
                        MIN_WORK_DURATION.as_secs() / 60
                    },
                    if is_seconds_mode {
                        MAX_WORK_DURATION.as_secs()
                    } else {
                        MAX_WORK_DURATION.as_secs() / 60
                    },
                    if is_seconds_mode {
                        "seconds"
                    } else {
                ...
                || short_break_secs > MAX_SHORT_BREAK_DURATION.as_secs()
            {
                return Err(format!(
                    ...
                    if is_seconds_mode {
                        MIN_SHORT_BREAK_DURATION.as_secs()
                    } else {
                        MIN_SHORT_BREAK_DURATION.as_secs() / 60
                    },
                    if is_seconds_mode {
                        MAX_SHORT_BREAK_DURATION.as_secs()
                    } else {
                        MAX_SHORT_BREAK_DURATION.as_secs() / 60
                    },
                    if is_seconds_mode {
                        "seconds"
                    } else {
                ...
                || long_break_secs > MAX_LONG_BREAK_DURATION.as_secs()
            {
                return Err(format!(
                    ...
                    if is_seconds_mode {
                        MIN_LONG_BREAK_DURATION.as_secs()
                    } else {
                        MIN_LONG_BREAK_DURATION.as_secs() / 60
                    },
                    if is_seconds_mode {
                        MAX_LONG_BREAK_DURATION.as_secs()
                    } else {
                        MAX_LONG_BREAK_DURATION.as_secs() / 60
                    },
                    if is_seconds_mode {
                        "seconds"
                    } else {
            ...
            if sessions_until_long_break.get()
                < MIN_SESSIONS_UNTIL_LONG_BREAK as usize
                || sessions_until_long_break.get()
                    > MAX_SESSIONS_UNTIL_LONG_BREAK as usize
            {
                return Err(format!(
