...
export function App() {
  ...
  useEffect(() => {
    ...
    const unlistenTaskList = listen(AppEvents.TaskListUpdated, () => loadTasks())
    const unlistenTaskActive = listen(AppEvents.TaskActiveChanged, () => loadTasks())
    const unlistenTaskCompleted = listen(AppEvents.TaskCompleted, () => loadTasks())
    const unlistenTaskProgress = listen(AppEvents.TaskProgressUpdated, () => loadTasks())
    ...
    const unlistenTimerTick = listen(AppEvents.TimerTick, (event) => {
      applyTick(event.payload as { task_id: string; phase: string; remaining_seconds: number })
    })

    // State-change events re-fetch from backend for accurate full state
    const unlistenTimerStatus = listen(AppEvents.TimerStatusChanged, () => fetchTimer())
    ...
    const unlistenTimerPaused = listen(AppEvents.TimerPaused, () => fetchTimer())
    const unlistenTimerResumed = listen(AppEvents.TimerResumed, () => fetchTimer())

    const unlistenScreenBlocker = listen(AppEvents.ScreenBlockerActivate, (event) => {
  ...
  if (!ready) {
    return (
      <div className="flex h-screen w-full items-center justify-center bg-gradient-to-br from-indigo-50 via-white to-purple-50 dark:from-gray-950 dark:via-gray-900 dark:to-indigo-950">
        <span className="text-sm text-muted-foreground animate-pulse">Starting Pomotoro…</span>
      </div>
    )
  }
    ...
    <>
      <div className="flex h-screen w-full overflow-hidden font-sans bg-gradient-to-br from-indigo-50 via-white to-purple-50 dark:from-gray-950 dark:via-gray-900 dark:to-indigo-950 text-foreground transition-colors duration-300">
        ...
        <main className="flex-1 overflow-y-auto p-6 md:p-10">
          {page === 'timer' && <ErrorBoundary><TimerPage /></ErrorBoundary>}
          {page === 'tasks' && <ErrorBoundary><TasksPage onNavigate={setPage} /></ErrorBoundary>}
          {page === 'settings' && <ErrorBoundary><SettingsPage /></ErrorBoundary>}
        </main>
      </div>
      {isBlocking && <ScreenBlocker message={blockingMessage} onDismiss={handleDismissBlocker} />}
    </>
  )
}
