...
import { Play, Pause, RotateCcw, SkipForward, CheckCircle, RefreshCw } from 'lucide-react'
...
function getPhaseDuration(phase: Phase, cfg: TimerConfiguration | null | undefined): number {
  switch (phase) {
    case Phase.ShortBreak: return cfg?.short_break_duration ?? 300
    case Phase.LongBreak: return cfg?.long_break_duration ?? 900
    default: return cfg?.work_duration ?? 1500
  }
}
...
export function TimerPage() {
  const { timer, error: timerError, start, pause, resume, reset, skip, fetchTimer } = useTimerStore()
  const { tasks, getActiveTask, completeActiveTask, resetActiveTask } = useTaskStore()
  ...
  const progress = phaseDuration > 0 ? Math.min(1, Math.max(0, remaining / phaseDuration)) : 1
  ...
  let dotFilled = 0
  if (contextTask) {
    const hasFixedSessions = !contextTask.default && (contextTask.max_sessions ?? 0) > 0
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-full gap-5 py-10">
      {/* Phase label */}
      <span className={`text-xs font-bold uppercase tracking-[0.2em] ${PHASE_COLOR[phase]}`}>
        {PHASE_LABEL[phase]}
      </span>
      ...
      {contextTask && !contextTask.default && (
        <div className="flex items-center gap-2.5 px-4 py-2 rounded-full bg-card border border-border shadow-sm max-w-xs truncate">
          ...
          <span className="text-sm font-medium truncate">{contextTask.name}</span>
      ...
      {!contextTask && !running && !paused && (
        <span className="text-xs text-muted-foreground">Select a task to start</span>
      )}
