...
use domain::{
    ConfigRepository, GeneralConfig, Phase, TaskId, TaskRepository, Timer,
    TimerStatus, event_names::ui_listeners,
};
use infra::adapters::{
    TimerRepositoryArc, events::mem_event_bus::EventPublisherArc,
};
use serde_json;
use tauri::{
...
use usecases::timer::{
    StartTimerPhaseCmd, pause_timer_phase, reset_timer_to_idle,
    start_timer_phase,
};
...
const MI_START: &str = "start";
const MI_PAUSE: &str = "pause";
const MI_RESET: &str = "reset";
...
pub struct TrayMenuHandles {
    ...
    pub start: Arc<MenuItem<Wry>>,
    pub pause: Arc<MenuItem<Wry>>,
    pub reset: Arc<MenuItem<Wry>>,
}
...
pub fn build_tray(app: &AppHandle) -> tauri::Result<()> {
    ...
    let initial_icon = match app.default_window_icon() {
        Some(base) => overlay_countdown(base, None),
        None => Image::new_owned(vec![0; 4], 1, 1),
    };
...
fn build_menu(app: &AppHandle) -> tauri::Result<(Menu<Wry>, TrayMenuHandles)> {
    ...
    let start = MenuItem::with_id(app, MI_START, "Start", true, None::<&str>)?;
    let pause = MenuItem::with_id(app, MI_PAUSE, "Pause", true, None::<&str>)?;
    let reset = MenuItem::with_id(app, MI_RESET, "Reset", true, None::<&str>)?;
    ...
    let menu = Menu::with_items(
        app,
        &[&toggle, &sep1, &start, &pause, &reset, &sep2, &quit],
    )?;
...
fn on_menu_event(app: &AppHandle, event: MenuEvent) {
    match event.id().as_ref() {
        ...
        MI_START => menu_start(app),
        MI_PAUSE => menu_pause(app),
        MI_RESET => menu_reset(app),
        MI_QUIT => quit_app(app),
        _ => {}
    }
}

fn menu_start(app: &AppHandle) {
    let Some(task_id) = task_id_for_action(app) else {
        return;
    };
    let Some(task_repo) = app
        .try_state::<Arc<dyn TaskRepository + Send + Sync>>()
        .map(|s| s.inner().clone())
    else {
        return;
    };
    let Some(timer_repo) = app
        .try_state::<TimerRepositoryArc>()
        .map(|s| s.inner().clone())
    else {
        return;
    };
    let Some(event_publisher) = app
    ...
    tauri::async_runtime::spawn(async move {
        if let Err(e) =
            start_timer_phase(task_repo, timer_repo, event_publisher, cmd).await
        {
            log::error!("Tray start failed: {}", e);
        }
        let _ = refresh(&app, None);
    });
}

fn menu_pause(app: &AppHandle) {
    let Some((timer, _)) = load_state(app) else {
        return;
    };
    let Some(task_id) = timer.task_id() else {
        return;
    };
    let remaining_seconds = timer.remaining_seconds(None);
    ...
    let app = app.clone();
    tauri::async_runtime::spawn(async move {
        if let Err(e) = pause_timer_phase(
            task_id,
            remaining_seconds,
            task_repo,
            timer_repo,
        ...
        .await
        {
            log::error!("Tray pause failed: {}", e);
        }
        let _ = refresh(&app, None);
    });
}

fn menu_reset(app: &AppHandle) {
    let Some(task_id) = task_id_for_action(app) else {
        return;
    };
    let Some(task_repo) = app
    ...
    tauri::async_runtime::spawn(async move {
        if let Err(e) =
            reset_timer_to_idle(task_id, task_repo, timer_repo, event_publisher)
                .await
        {
            log::error!("Tray reset failed: {}", e);
        }
        let _ = refresh(&app, None);
    });
}

/// Extract the bound task_id for menu actions (Start/Reset).
fn task_id_for_action(app: &AppHandle) -> Option<TaskId> {
    load_state(app).and_then(|(timer, _)| timer.task_id())
}
...
/// Subscribe to timer + config events so the tray stays in sync.
...
/// Re-read timer + general config and update title/icon/tooltip + menu state.
...
pub fn refresh(
    ...
    let Some((timer, general)) = load_state(app) else {
        return Ok(());
    };
    ...
    // currently stopped. This is what gates Start/Reset availability.
    ...
    if let Some(tray) = app.tray_by_id(TRAY_ID) {
        ...
        if is_linux() {
            ...
            let icon = match app.default_window_icon() {
                Some(base) => overlay_countdown(base, countdown.as_deref()),
                None => Image::new_owned(vec![0; 4], 1, 1),
            };
    ...
    if let Some(h) = app.try_state::<TrayMenuHandles>() {
        ...
        // No active task => disable everything except toggle/quit.
        let _ = h.toggle.set_enabled(true);
        let _ = h.start.set_enabled(has_task && !running && !paused);
        let _ = h.pause.set_enabled(running);
        let _ = h.reset.set_enabled(has_task);
    }
...
/// Load the current timer and general config from managed Tauri state.
fn load_state(app: &AppHandle) -> Option<(Timer, GeneralConfig)> {
