...
use crate::task::{
    CreateTaskCmd, SetDefaultTaskCmd, SwitchActiveTaskCmd, create_task,
    set_default_task, switch_active_task,
};
...
pub async fn bootstrap(
    ...
    let task_for_timer = if !task.is_completed() {
        ...
        match task_repo.get_incomplete_tasks().await {
            Ok(incomplete_tasks) if !incomplete_tasks.is_empty() => {
                info!(
                    "Found {} incomplete tasks, using the first one",
    ...
    if let Some(active_task) = task_for_timer {
        ...
        .await
        {
            ...
            info!(
                "Continuing without active task. User can select a task from UI."
            );
        } else {
            info!(
                "Timer task switched successfully to: {}",
                active_task.name()
            );
        }
    } else {
