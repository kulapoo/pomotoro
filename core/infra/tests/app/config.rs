...
use domain::{
    Config, ConfigRepository, EventPublisher, TaskRepository,
    TimerConfiguration,
    config::events::{ConfigReset, ConfigUpdated},
    event_names,
    shared_kernel::events::AppStarted,
};
use usecases::{UpdateConfigCmd, get_config, reset_config, update_config};
...
async fn config_should_initialize_on_app_start() {
    ...
    assert_eq!(
        config.timer.long_break_duration,
...
async fn should_update_timer_durations_in_config() {
    ...
    assert_eq!(
        final_config.timer.work_duration,
        Duration::from_secs(30 * 60)
    );
    assert_eq!(
        final_config.timer.short_break_duration,
...
async fn should_reset_config_to_factory_defaults() {
    ...
    let mut modified_config =
        get_config(ctx.config_repo.clone()).await.unwrap();
    ...
    let result = reset_config(&config_repo, &event_bus).await;
    ...
    assert_eq!(
        reset_config.timer.work_duration,
        Duration::from_secs(25 * 60)
    ); // 25 minutes default
    assert_eq!(
        reset_config.timer.short_break_duration,
        Duration::from_secs(5 * 60)
    ); // 5 minutes default
...
async fn config_changes_should_apply_to_new_timer_sessions() {
    // GIVEN
    let ctx =
        setup_ctx("config_changes_should_apply_to_new_timer_sessions").await;
    ...
    assert_eq!(
        updated_from_result.timer.work_duration,
        Duration::from_secs(20 * 60)
    );
    assert_eq!(
        updated_from_result.timer.short_break_duration,
    ...
    assert_eq!(
        updated_config.timer.work_duration,
        Duration::from_secs(20 * 60)
    );
    assert_eq!(
        updated_config.timer.short_break_duration,
        Duration::from_secs(3 * 60)
    );
}
...
async fn config_state_should_persist_across_restarts() {
    ...
    // Verify it was saved
    let config_before_restart =
        get_config(ctx.config_repo.clone()).await.unwrap();
    assert_eq!(
        config_before_restart.timer.work_duration,
        Duration::from_secs(30 * 60)
    );
    ...
    let config_after_restart =
        get_config(new_ctx.config_repo.clone()).await.unwrap();

    // THEN
    assert_eq!(
        config_after_restart.timer.work_duration,
        Duration::from_secs(30 * 60)
    );
    assert_eq!(
        config_after_restart.timer.long_break_duration,
        Duration::from_secs(20 * 60)
    );
...
async fn should_validate_config_boundaries() {
    ...
    assert!(
        result2.is_err(),
        "Should reject work duration exceeding 3 hours"
    );
}
...
async fn should_update_multiple_config_sections_simultaneously() {
    // GIVEN
    let ctx =
        setup_ctx("should_update_multiple_config_sections_simultaneously")
            .await;
    ...
    assert_eq!(
        updated_config.timer.long_break_duration,
        original_config.timer.long_break_duration
    );
...
async fn should_preserve_unchanged_fields_on_partial_update() {
    // GIVEN
    let ctx =
        setup_ctx("should_preserve_unchanged_fields_on_partial_update").await;
    ...
    let partial_timer_config = TimerConfiguration {
        ...
        sessions_until_long_break: initial_timer_config
    ...
    assert_eq!(
        final_config.timer.long_break_duration,
        Duration::from_secs(25 * 60)
    );
...
async fn should_handle_concurrent_config_updates() {
    ...
    let is_first_update = final_config.timer.work_duration
        == Duration::from_secs(20 * 60)
        && final_config.timer.sessions_until_long_break == 4;

    let is_second_update = final_config.timer.work_duration
        == Duration::from_secs(30 * 60)
        && final_config.timer.sessions_until_long_break == 6;

    assert!(
        is_first_update || is_second_update,
...
async fn should_handle_invalid_sessions_until_long_break() {
    // GIVEN
    let ctx =
        setup_ctx("should_handle_invalid_sessions_until_long_break").await;
    ...
    assert!(
        result.is_err(),
        "Should reject zero sessions_until_long_break"
    );
...
async fn should_not_allow_config_update_during_active_timer() {
    ...
    let ctx =
        setup_ctx("should_not_allow_config_update_during_active_timer").await;
    ...
    assert!(
        result.is_ok(),
        "Config should be updatable during timer session"
    );
}
...
async fn should_recover_from_failed_config_operations() {
    ...
    assert!(
        ...
        "Should accept valid config after failed attempt"
    );
...
async fn should_maintain_config_consistency_after_reset_and_update_cycle() {
    // GIVEN
    let ctx = setup_ctx(
        "should_maintain_config_consistency_after_reset_and_update_cycle",
    )
    .await;
