...
use domain::{
    Config, Phase, TaskRepository, TaskStatus, TimerConfiguration,
    TimerRepository, TimerState, TimerStatus,
};
use usecases::{
...
use crate::{
    TaskBuilder,
    utils::{setup::setup_ctx, task::get_active_task, timer::get_timer},
};
...
async fn should_skip_from_work_to_break_phase() {
    ...
    let is_break_phase =
        matches!(state_after_skip, TimerState::ShortBreak { .. });
...
async fn pause_and_resume_should_maintain_timer_state() {
    ...
    assert_eq!(
        remaining_before_pause,
        15 * 60,
        "Should have 15 minutes remaining after 10 minutes of work"
    );

    assert!(pause_result.is_ok(), "Failed to pause timer");
    assert_eq!(
        pause_result.unwrap().status(),
        TimerStatus::Paused,
    ...
    assert_eq!(
        ...
        TimerStatus::Running,
        "Timer should be running after resume"
    );

    // Duration shouldn't change while paused
    assert_eq!(
        remaining_after_resume, remaining_before_pause,
...
async fn task_with_custom_settings_overrides_defaults() {
    ...
    assert_eq!(
        initial_duration,
        15 * 60,
        "Should use custom work duration of 15 minutes"
    );

    assert!(
        complete_work_result.is_ok(),
        "Failed to complete work phase"
    );
    assert_eq!(
        break_phase,
        Phase::ShortBreak,
    ...
    assert_eq!(
        break_duration,
        3 * 60,
        "Should use custom break duration of 3 minutes"
    );
}
...
async fn should_filter_tasks_by_status() {
    ...
    assert!(
        queued_names.contains(&"Queued task 2".to_string()),
        "Should find 'Queued task 2'"
    );
    ...
    assert!(
        active_tasks.len() >= 1,
        "Should find at least 1 active task"
    );
...
async fn should_trigger_long_break_after_4_work_sessions() {
    let ctx =
        setup_ctx("should_trigger_long_break_after_4_work_sessions").await;
    ...
    // Assert
    assert!(
        complete_4th_result.is_ok(),
        "Failed to complete 4th work session"
    );
    assert!(
        is_long_break,
        "Should trigger long break after 4 work sessions"
    );
...
async fn should_emit_tick_events_every_second() {
    ...
    for _i in 0..5 {
        ...
        for event in events {
            if event.event_type() == "Tick" {
                if let Some(tick) =
                    event.as_any().downcast_ref::<domain::timer::events::Tick>()
                {
    ...
    if let Some(last_tick) = tick_events.last() {
        assert_eq!(
            last_tick.remaining_seconds,
            (25 * 60) - 5,
            "Last tick should show 5 seconds elapsed"
        );
    ...
    assert_eq!(
        final_remaining,
        (25 * 60) - 5,
        "Should have decremented by 5 seconds"
    );
}
...
async fn should_give_long_break_after_skipping_4_work_sessions() {
    let ctx =
        setup_ctx("should_give_long_break_after_skipping_4_work_sessions")
            .await;
