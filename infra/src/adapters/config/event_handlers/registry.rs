...
use super::{ConfigUpdatedHandler, ConfigResetHandler};
...
pub fn register_config_handlers(
    ...
    event_bus.subscribe(Box::new(ConfigUpdatedHandler::new(
        emitter.clone(),
    )))?;

    // Register ConfigReset handler
    event_bus.subscribe(Box::new(ConfigResetHandler::new(
        emitter.clone(),
    )))?;
