...
pub fn register_timer_handlers(
    ...
    event_bus.subscribe(Box::new(TimerStartedHandler::new(app_handle.clone())))?;
    event_bus.subscribe(Box::new(TimerTickHandler::new(app_handle.clone())))?;
    event_bus.subscribe(Box::new(PhaseCompletedHandler::new(app_handle.clone())))?;
    event_bus.subscribe(Box::new(PhaseSkippedHandler::new(app_handle.clone())))?;
    event_bus.subscribe(Box::new(TimerStatusChangedHandler::new(app_handle.clone())))?;
...
pub fn unregister_timer_handlers(
    ...
    event_bus.clear_handlers_for_type(TypeId::of::<TimerStatusChangedHandler>())?;
