...
use crate::adapters::{
    RodioAudioService, TimerService, audio::InMemoryAudioLibraryService,
};
...
pub async fn bootstrap(
    app_handle: AppHandle,
) -> Result<AppRegistry, BootstrapError> {
    ...
    let audio_service = Arc::new(
        RodioAudioService::new()
            .map_err(|e| BootstrapError::AudioInit(e.to_string()))?,
    );
    ...
    let timer_service: Arc<dyn DomainTimerService + Send + Sync> =
        Arc::new(TimerService::new_with_services(
