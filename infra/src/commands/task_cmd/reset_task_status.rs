...
use log::info;
use usecases::task::reset_task_status as reset_task_status_usecase;
...
pub async fn reset_task_status(
    ...
    info!(
        "Resetting task status: id={}, reset_sessions={}",
    ...
    reset_task_status_usecase(&task_repo, task_id_parsed, reset_sessions)
        .await
        .with_context(|| format!("Failed to reset status for task: {}", task_id))
        .map_err(|e| {
    ...
    info!(
        "Successfully reset task status: id={}, new_status={:?}",
        task_id, task.status
    );
