...
async fn test_config_repository_basic_operations() {
    ...
    assert_eq!(
        default_config.task_defaults.work_duration,
        Duration::from_secs(25 * 60)
    );
    assert_eq!(
        default_config.task_defaults.short_break_duration,
        Duration::from_secs(5 * 60)
    );
    assert_eq!(
        default_config.task_defaults.long_break_duration,
...
fn test_config_builder_patterns() {
    ...
    assert_eq!(
        fast_config.task_defaults.work_duration,
        Duration::from_secs(5)
    );
    assert_eq!(
        fast_config.task_defaults.short_break_duration,
        Duration::from_secs(2)
    );
    assert_eq!(
        fast_config.task_defaults.long_break_duration,
    ...
    assert_eq!(
        slow_config.task_defaults.work_duration,
        Duration::from_secs(60 * 60)
    );
    assert_eq!(
        slow_config.task_defaults.short_break_duration,
        Duration::from_secs(30 * 60)
    );
    assert_eq!(
        slow_config.task_defaults.long_break_duration,
...
async fn test_per_task_default_configuration() {
    ...
    assert_eq!(
        saved_config.task_defaults.work_duration,
        Duration::from_secs(25 * 60)
    );
    assert_eq!(
        saved_config.task_defaults.short_break_duration,
        Duration::from_secs(5 * 60)
    );
    assert_eq!(
        saved_config.task_defaults.long_break_duration,
        Duration::from_secs(15 * 60)
    );
...
async fn test_custom_timing_configurations() {
    ...
    assert_eq!(
        saved_short.task_defaults.work_duration,
        Duration::from_secs(5 * 60)
    );
    assert_eq!(
        saved_short.task_defaults.short_break_duration,
    ...
    assert_eq!(
        saved_long.task_defaults.work_duration,
        Duration::from_secs(60 * 60)
    );
    assert_eq!(
        saved_long.task_defaults.short_break_duration,
        Duration::from_secs(30 * 60)
    );
    assert_eq!(
        saved_long.task_defaults.long_break_duration,
        Duration::from_secs(60 * 60)
    );
}
...
async fn test_screen_blocking_configuration() {
    ...
    let no_blocking_config =
        TestConfigBuilder::new().with_screen_blocking(false).build();
...
async fn test_session_cycle_configuration() {
    ...
    config_repo
        .save_config(&single_session_config)
        .await
        .unwrap();
    ...
    config_repo
        .save_config(&extended_cycle_config)
        .await
        .unwrap();
    let saved_extended = config_repo.get_config().await.unwrap();
    assert_eq!(saved_extended.task_defaults.sessions_until_long_break, 8);
    assert_eq!(
        saved_extended.task_defaults.work_duration,
        Duration::from_secs(45 * 60)
    );
    assert_eq!(
        saved_extended.task_defaults.long_break_duration,
        Duration::from_secs(30 * 60)
    );
}
...
async fn test_configuration_persistence_patterns() {
    ...
    assert_eq!(
        saved_deep_focus.task_defaults.work_duration,
        Duration::from_secs(45 * 60)
    );
    ...
    assert_eq!(
        saved_quick.task_defaults.work_duration,
        Duration::from_secs(15 * 60)
    );
