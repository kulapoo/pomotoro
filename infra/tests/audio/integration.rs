use crate::audio::models::{
    AudioConfigBuilder, AudioTestAssertions, MockAudioManager,
};
use domain::PlaybackRequest;
...
async fn test_audio_background_playback() {
    ...
    let handle = audio_manager
        .play_background_audio("nature-sounds", 0.6)
        .unwrap();
...
fn test_audio_config_silent() {
    let config = AudioConfigBuilder::new().silent().build();
...
fn test_audio_config_custom_sounds() {
    let config = AudioConfigBuilder::new().with_custom_sounds().build();
...
async fn test_task_specific_audio_volumes() {
    ...
    let work_background_handle = audio_manager
        .play_background_audio("binaural-focus", 0.7)
        .unwrap();
    assert_eq!(
        audio_manager.get_playback_volume(&work_background_handle.id),
        Some(0.7)
    );

    let work_notification_handle = audio_manager
        .play_notification("subtle-chime", 0.7)
        .unwrap();
    assert_eq!(
        audio_manager.get_playback_volume(&work_notification_handle.id),
        Some(0.7)
    );
    ...
    let study_background_handle = audio_manager
        .play_background_audio("nature-sounds", 0.4)
        .unwrap();
    assert_eq!(
        audio_manager.get_playback_volume(&study_background_handle.id),
        Some(0.4)
    );

    let study_notification_handle =
        audio_manager.play_notification("soft-ping", 0.4).unwrap();
    assert_eq!(
        audio_manager.get_playback_volume(&study_notification_handle.id),
        Some(0.4)
    );
}
...
async fn test_toro_audio_background_soundscapes() {
    ...
    for soundscape in soundscapes {
        let handle = audio_manager
            .play_background_audio(soundscape, 0.5)
            .unwrap();
...
async fn test_audio_session_transitions() {
    ...
    let background_handle = audio_manager
        .play_background_audio("focus-sounds", 0.6)
        .unwrap();
    let _work_notification =
        audio_manager.play_notification("work-start", 0.6).unwrap();
    ...
    audio_manager.stop_background_audio();
    let _break_notification =
        audio_manager.play_notification("break-start", 0.6).unwrap();
    ...
    let new_background_handle = audio_manager
        .play_background_audio("focus-sounds", 0.6)
        .unwrap();
...
async fn test_per_task_volume_independence() {
    ...
    let loud_handle =
        audio_manager.play_notification("loud-chime", 0.9).unwrap();
    assert_eq!(
        audio_manager.get_playback_volume(&loud_handle.id),
        Some(0.9)
    );
    ...
    let quiet_handle = audio_manager
        .play_notification("soft-whisper", 0.2)
        .unwrap();
    assert_eq!(
        audio_manager.get_playback_volume(&quiet_handle.id),
        Some(0.2)
    );
}
...
async fn test_audio_volume_control_per_task() {
    ...
    let muted_handle =
        audio_manager.play_background_audio("ambient", 0.0).unwrap();
    assert_eq!(
        audio_manager.get_playback_volume(&muted_handle.id),
        Some(0.0)
    );
    ...
    let normal_handle = audio_manager
        .play_background_audio("normal-sounds", 0.6)
        .unwrap();
    assert_eq!(
        audio_manager.get_playback_volume(&normal_handle.id),
        Some(0.6)
    );
...
async fn test_audio_asset_management_per_task() {
    ...
    assert!(
        !library.assets.is_empty(),
        "Audio library should have assets loaded"
    );

    // Test loading custom soundscape
    let custom_handle = audio_manager
        .play_background_audio("binaural-focus", 0.5)
        .unwrap();
    ...
    let _chime_handle = audio_manager
        .play_notification("subtle-chime", 0.7)
        .unwrap();
    let _bell_handle =
        audio_manager.play_notification("gentle-bell", 0.6).unwrap();
...
async fn test_concurrent_task_audio_switching() {
    ...
    let task1_background = audio_manager
        .play_background_audio("task1-ambient", 0.5)
        .unwrap();

    assert!(audio_manager.is_playing(&task1_background.id));
    assert_eq!(
        audio_manager.get_playback_volume(&task1_background.id),
        Some(0.5)
    );

    // Switch to task2 - stop task1 audio and start task2 audio
    audio_manager.stop_background_audio(); // Stops task1 background
    let task2_background = audio_manager
        .play_background_audio("task2-ambient", 0.8)
        .unwrap();
    ...
    assert_eq!(
        audio_manager.get_playback_volume(&task2_background.id),
        Some(0.8)
    );
...
fn test_audio_config_builder_mvp2_features() {
    ...
    assert_eq!(
        deep_focus_config.background_sound,
    ...
    assert_eq!(
        energetic_config.background_sound,
        Some("upbeat-instrumental".to_string())
    );

    // Silent study configuration
    let silent_config = AudioConfigBuilder::new().silent().build();
