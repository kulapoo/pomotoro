...
use crate::timer::models::{TimerTestAssertions, TimerTestService};
use domain::{Phase, TaskRepository, timer::TimerService};
use std::time::Duration;

#[tokio::test]
async fn test_e2e_complete_work_session()
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let (old_phase, new_phase) =
        timer_service.skip_to_next_phase(Some(default_task)).await?;
...
async fn test_e2e_timer_persistence() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    assert_eq!(
        resumed_state.active_entity_id(),
        paused_state.active_entity_id()
    );
    // Remaining time should be close (within tolerance for test timing)
    let time_diff = (resumed_state.remaining_seconds() as i64
        - paused_state.remaining_seconds() as i64)
        .abs();
...
#[tokio::test]
async fn test_e2e_timer_with_multiple_tasks()
-> Result<(), Box<dyn std::error::Error>> {
    let timer_service = TimerTestService::new();
    ...
    // Switch to study task - need to stop timer first since we're in break
...
#[tokio::test]
async fn test_e2e_timer_reset_functionality()
-> Result<(), Box<dyn std::error::Error>> {
    let timer_service = TimerTestService::new();
    let task_repo = TaskTestRepository::with_default_task();
...
#[tokio::test]
async fn test_e2e_timer_performance_stress()
-> Result<(), Box<dyn std::error::Error>> {
    let timer_service = TimerTestService::new();
    let task_repo = TaskTestRepository::with_default_task();
    ...
    assert!(
        ...
        "Timer operations took too long: {elapsed:?}"
    );
