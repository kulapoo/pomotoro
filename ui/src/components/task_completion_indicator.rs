...
#[component]
pub fn TaskCompletionIndicator(task: ReadSignal<Option<Task>>) -> impl IntoView {
    ...
    let completion_text = move || {
        task.get()
            .map(|t| {
                if t.is_completed() {
                    format!("✓ Complete ({}/{})", t.current_sessions, t.max_sessions)
                } else {
                    format!("{}/{} sessions", t.current_sessions, t.max_sessions)
                }
            })
